# AHC014
暫定47.1M。貪欲してから焼きなまし。近傍は削除or長方形を変形→可能な限り挿入。挿入の評価関数を2個作った。挿入はbestじゃなくて重みでroulette-wheel-selection。タブーリストで削除頂点の挿入を数反復禁止したり指定の反復数改善がなければbestに戻した
...
# アイデア
- 今度こそ差分更新させる insertableを 頂点ごとに覚えておく？ constructを印を打った点3つから残り1点を探す感じにすると？
- もしくは重みが高い頂点から探していってもいいな…？
- 詰まったら0から再実装し直すのもいいな

# 方針7 9/27
サイクリングしたくない。削除した点をしばらく挿入しないようにタブーリストを用いることで多様化を狙っているので、サイクリングが起こるような挿入に対し、しばらく削除を禁止するようにタブーリストを使い、探索の集中化を狙う。（これ集中化か？）

削除禁止頂点は挿入を見て、決める（どうやって？）
まずその反復の挿入長方形を覚える 訪問回数で見る？見通しがよくない気がする ratioだな
> 分母は: (1)総反復回数 (2)分子の和（または平均）(3)分子の最大値
(3)よくないか？


タブーリストをもとに削除しないようにする（した）　提出46.7M。seed=0は84万点

# 方針9 9/27
- 頂点じゃなくて長方形で禁止するのはどうなん？
- FIXRATIOを範囲にするのは？

# アイデア
- 長方形を選んで、他の長方形にする近傍は？←めっちゃ強そう　変えたらできるだけ挿入
- 上位k個を保存して新しい解を作る
- 新しい解を作るときに、基本含まれるやつを削除禁止にする
- 訪問点を最大化するような評価関数も用意して交互に探索